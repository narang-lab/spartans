...
{	printf("Usage: %s [options], where options could be:\n",name);
	printf("\t-h --help               help (this output)\n");
	printf("\t-i --input <filename>   input HDF5 file (required)\n");
	printf("\t-o --output <filename>  output HDF5 file (required)\n");
	printf("\n");
}
...
int main(int argc, char** argv)
{
	...
	while(true)
	{	int c = getopt_long(argc, argv, "hi:o:", long_options, 0);
		if (c == -1) break; //end of options
		switch (c)
		{	case 'h': printUsage(argv[0]); return 0;
			case 'i': inFile.assign(optarg); break;
			case 'o': outFile.assign(optarg); break;
			default: printUsage(argv[0]); MPI_Abort(MPI_COMM_WORLD, 1);
		}
	}
	if(!inFile.length()) { printf("Must specify input HDF5 filename.\n"); printUsage(argv[0]); MPI_Abort(MPI_COMM_WORLD, 1); }
	if(!outFile.length()) { printf("Must specify output HDF5 filename.\n"); printUsage(argv[0]); MPI_Abort(MPI_COMM_WORLD, 1); }
	...
	//--- mesh:
	MeshTransport mt;
	mt.readInputs(MPI_COMM_WORLD, fid);
	...
	kArr = h5readMatrix<3,double>(MPI_COMM_WORLD, fid,"k"); //decay wavevectors
	BiArr = h5readVectorArray<double>(MPI_COMM_WORLD, fid,"Bi");//body inputs
	SiArr = h5readMatrixArray<3,double>(MPI_COMM_WORLD, fid,"Si");//surface inputs
	...
	MPI_Allreduce(MPI_IN_PLACE, nEventsProc.data(), nProcs, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(MPI_IN_PLACE, binEvents.data(), nBins, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(MPI_IN_PLACE, procStart.data(), nBins, MPI_INT, MPI_MIN, MPI_COMM_WORLD);
	MPI_Allreduce(MPI_IN_PLACE, procStop.data(), nBins, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
	...
	{	std::vector<double>& Bi = BiArr[ie];
		...
		{	for(int jProc=procStart[ie]; jProc<=procStop[ie]; jProc++)
				if(jProc != iProc)
				{	MPI_Send(Bi.data(), Bi.size(), MPI_DOUBLE, jProc, ie, MPI_COMM_WORLD);
					MPI_Send(Si.data(), Si.size()*3, MPI_DOUBLE, jProc, ie, MPI_COMM_WORLD);
				}
		...
		{	Bi.resize(mt.verts.size());
			...
			MPI_Recv(Bi.data(), Bi.size(), MPI_DOUBLE, MPI_ANY_SOURCE, ie, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			MPI_Recv(Si.data(), Si.size()*3, MPI_DOUBLE, MPI_ANY_SOURCE, ie, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
	...
	{	//For each bin, the lowest involved process is responsible for final procesing and output:
		...
		{	//Recieve contributions from other processes:
			...
			{	MPI_Recv(OBoBuf.data(), mt.verts.size(), MPI_DOUBLE, jProc, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				MPI_Recv(SoBuf.data(), 3*mt.extSindex.size(), MPI_DOUBLE, jProc, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				MPI_Recv(inOutBuf.data(), mt.extSindex.size(), MPI_DOUBLE, jProc, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		...
		{	MPI_Send(OBoArr[ie].data(), mt.verts.size(), MPI_DOUBLE, procStart[ie], 0, MPI_COMM_WORLD);
			MPI_Send(SoArr[ie].data(), 3*mt.extSindex.size(), MPI_DOUBLE, procStart[ie], 1, MPI_COMM_WORLD);
			MPI_Send(inOut[ie].data(), mt.extSindex.size(), MPI_DOUBLE, procStart[ie], 1, MPI_COMM_WORLD);
	...
	MPI_Allreduce(MPI_IN_PLACE, &nFailed, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
	...
	H5Pclose(plid);
	//--- write data:
	h5writeVectorArray(MPI_COMM_WORLD, fid, "Bo", BoArr); //Bo data
	h5writeMatrixArray(MPI_COMM_WORLD, fid, "So", SoArr); //So data
	h5writeVectorArray(MPI_COMM_WORLD, fid, "inOut", inOut); //InOut data
	h5writeMatrix(MPI_COMM_WORLD, fid, "SoIndex", mt.extSindex); //vertex indices per face in So output
	//--- write diagnostic integrals:
	MPI_Allreduce(MPI_IN_PLACE, intBSiArr.data(), nBins, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	h5writeVector(MPI_COMM_WORLD, fid, "intBSi", intBSiArr); //Bi+Si; spatial integral
	//--- close file:
	H5Fclose(fid);
	//watchOutput.stop();
	...
	//Sum rule check:
	printf("\nSum rule check:\n"); fflush(stdout);
	MPI_Allreduce(MPI_IN_PLACE, intSoArr.data(), nBins, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(MPI_IN_PLACE, intBoArr.data(), nBins, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	...
	//MPI_Allreduce(MPI_IN_PLACE, &nCalls, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
	//MPI_Allreduce(MPI_IN_PLACE, &Ttot, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
